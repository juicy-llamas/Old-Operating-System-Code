...
print_length:			;esi == stuff, cx == length (chars) to print for
	push edi
	mov edi, dword [cur_mem]	;virtually identical to print_str, except we have a different looping mechanism
	push ax						
	push bx
	xor bx, bx
...
scroll_by_amount:	;amount (of lines) to scroll in cl
	push ebx
	mov edi, 0B8000H
	mov ebx, 50H
	.loop:
...
change_screen_color:			;re-write the screen with a different color.
	push edi
	mov edi, 0B8000H			;clear all visible screen: from the beginning
	.loop:
		cmp edi, 0B8FA0H		;the amount of bytes of memory in the screen: 80 columns of chars * 25 rows of chars * 2 bytes PER char = 4000 bytes.
...
skip_print:		;esi == string, ah == char to skip_print
	...
	mov edi, dword [cur_mem]	;move our current position
	push bx
	mov bl, byte [curcolor]	;and our current color
	.loop:
		cmp edi, 0B8FA0H
		jge .scroll
		mov al, byte [esi]
