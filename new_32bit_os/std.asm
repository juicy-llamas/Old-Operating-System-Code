...
num_to_hex_small:	;eax == number, edi == return buffer. number <= 4 bytes (it has to fit in ecx).
	push ebx		;buffer has to be 8 bytes, function will fill upper bytes with zeroes.
	push cx		;save previous registers
	push edx
	mov ebx, eax
	mov edx, 0F0000000H		;get top byte 1st
	mov cl, 28				
	.convert:
		and eax, edx		;get cur nibble
		shr eax, cl		;move nibble to bottom
		shr edx, 4
		sub cl, 4
		js .done		;if signed value, then we're done
		cmp al, 9
		ja .A_F
	.zero_nine:
		add al, 30H
