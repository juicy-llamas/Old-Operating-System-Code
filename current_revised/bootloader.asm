...
.read_sector: dw 0xFFFF
.offset: dw 0x7E00			;we load the kernel right next to the bootloader, which is very convinent for us since we don't have to change our origin :)
.segment: dw 0
.end: dq 1
...
gdt:					;GDT GUIDE !!!!!!!!
	...
	dw 0xFFFF			;this is the max we can set our segment registers (so we can load highest potental value in there)
	dw 0				;this is the BASE starting address (0)
	db 0				;continuation of last section (with BASE starter address)
	...
	db 0				;last byte of starting address for BASE that was before the two 'magical tables' (wtf is this ordering...)
						;data descriptor (or whatever)
	dw 0xFFFF			;same thing basically
	dw 0				
	db 0				
...
setup_protected_mode:
	mov ax, 0x3			;setup VGA graphics mode for ease
	int 0x10				
	
	in al, 0x92			;enabling A20 gate via motherboard port 146 (somehow, when you logical or the second bit of port 146 (92H), then you enable A20. Depends on mother board, obviously)
	...
	out 0x92, al 		;this should work for most, though
	...
	jmp 0x08:clear		;far jump to clear all of the registers (and main code)
