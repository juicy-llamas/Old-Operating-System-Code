...
key_array_shift:		;because im lazy/it's better performance to just browse an array rather than create a tree of cases that you have to manage
	...
	db 0	;print screen button	
	db 0	;BOTH alts			
	db ' '	;spacebar					for caps lock, we just use the no shift array and add 0x20 to the character (since they are lined up like that)
	db 0	;caps lock
	db 0	;function keys 1-10		;59/3B
	db 0
	db 0
...
;the controller that controls the keyboard (encoder) is port 0x64
...
main_keyboard_handler:		;in case the user wants to create a custom keyboard handler (say for a video game), this pointer will be stored in user accesible space, but of course, you cannot overwrite the handler to disk. only in memory.
	xor eax, eax
	...
	cmp al, 0x80		;if a key is released, check if it is a special key
	jae .cmp80
	...
	je .print_in_shift
	cmp [caps_lock_stat], byte 0xFF			;if the caps lock (or shift) is engaged, browse from the caps lock key array
	je .print_in_caps
	cmp [alt_stat], byte 1
	je .alt_hand
