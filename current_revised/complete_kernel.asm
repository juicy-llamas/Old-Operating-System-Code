...
org 0x7C00				;our entry point in memory
...
clear:
	mov ax, 0x10			;move all segment registers to the DATA segment and say goodbye to them FOREVER!!!
	mov ds, ax			;unless we need extra descriptors for apps (which we are going to)
	mov es, ax
	mov fs, ax
	mov gs, ax
	mov ss, ax
	mov esp, 0xE000		;move the stack pointer (points to the current stack location) to a distant location so as not to interfere (this will be edited later)...
...
%include 'std.asm'
%include 'timer.asm'
%include 'old/command_structure.asm'
...
times 0x10000-($-$$) db 0		;pad with zeros (i just chose this number because that would make my program exactly 1.44 MB, aka the amount of space on a floppy drive)
