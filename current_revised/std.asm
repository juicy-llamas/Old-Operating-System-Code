...
hex_to_num_small:	;esi == input, ecx == return string, ah == optional termination. small means <= 4 bytes.
...
num_to_hex_small:	;eax == number, edi == return buffer, number <= 4 bytes (it has to fit in eax).
	push ebx		
	push edx
	mov ebx, 0xF0000000
	mov edx, eax
...
binary_search:	;esi == list of information to search through, eax/ax/al == value to search for, cl == size of element in bytes (1 byte, 2 bytes, or 4 bytes), ebx == TOTAL size of list, trashes edx. Searches through an ordered list for the value in eax/ax/al. If not found, sets the carry flag.
	;TODO
...
strlen:		;esi == string, ah == optional termination, trashes ecx. string must be less than 2^32 chars (which is perfectly reasonable, i would think)
	xor ecx, ecx
	.loop:
		cmp [esi], byte 0
		je .done
